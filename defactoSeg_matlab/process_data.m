...
fn_in = 'C:\Temp\slices.mat';
fn_out = 'C:\Temp\slices2.mat';
...
[X,Xm,vmin,vmax] = x_normalize( tmp.X, setId );
fprintf('done\n');
% X = tmp.X;
...
function [x,xMean,vmin,vmax] = x_normalize(x, setId)
x = single(x);
...
% dynamic range AFTER taking out mean
vmin = min(x(:));
vmax = max(x(:));

% linearly map to [-1,+1] without clapping. Simply the analysis of all the
% pixels
ix = (x <= 0.0);
x(ix) = x(ix) ./ abs(vmin);
x(~ix) = x(~ix) ./ abs(vmax);
